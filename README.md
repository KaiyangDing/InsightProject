...
>
> 🚧 进行中的个人项目 · **Week 1–4 已完成** · LLM 编排器自主协调"取数 → 分析 → 审查 → 报告"，配 **LLM-as-judge 评测** 与 **Streamlit 网页 demo**。
...
- ✅ **核心逻辑全测试** —— pytest（41 个），用"假 LLM"做确定性测试；Docker 相关测试无环境时自动跳过。
...
├── tests/                 # pytest（纠错 / 护栏 / EX / 代码执行 / 分析 / 编排 / judge）
...
# 6. 网页 demo（Streamlit：输入问题 → 多智能体编排 → 报告 + 图表 + 轨迹）
...
```bash
uv run pytest -q     # 41 passed（Docker 相关测试无环境时自动 skip）
```
