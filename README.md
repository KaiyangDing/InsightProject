...
>
> 🚧 进行中的个人项目 · **Week 1–3 已完成** · LLM 编排器自主协调"取数 → 分析 → 审查 → 报告"。
...
```
            ...
            └──┬──────────────┬─────────┬────┘
  run_sql 工具 ▼  analyze_data 工具 ▼  终答前过闸 ▼
  ┌──────────────┐ ┌──────────────────┐ ┌──────────────────┐
  │ Text2SQLAgent│ │ CodeAnalysisAgent │ │   CriticAgent    │
  │ NL→SQL→只读   │ │ 沙箱 pandas/画图   │ │ 忠实性审查        │
  │ 自我纠错      │ │ 自我纠错           │ │ 不通过→打回重写    │
  └──────┬───────┘ └─────────┬────────┘ └──────────────────┘
...
> SQL / 分析两个子 agent 都继承 `SelfCorrectingAgent`（纠错循环只写一遍）；编排器靠**鸭子类型**接 Critic（只依赖 `.review()`，不 import）。
...
# 5. 多智能体编排 demo（supervisor 自主调度 SQL + 分析 + 审查）
uv run scripts/demos/orchestrate.py "把各品类销售额画成柱状图，并指出最高的品类"
```
...
```bash
uv run pytest -q     # 32 passed（Docker 相关测试无环境时自动 skip）
```
