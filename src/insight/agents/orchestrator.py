...
- run_sql 已把完整结果存入工作区。需要对全部数据做聚合/对比/排序/画图时，调用 analyze_data（它能读到全量）；切勿为了"看全数据"而重复 run_sql 同一张查询。
- 需要按某维度统计/分组/对比/找极值时，直接让 run_sql 在 SQL 里聚合（GROUP BY 出每组的统计量），不要拉原始明细再交给 analyze_data 去分组——run_sql 有行数上限，明细会被截断、导致按组统计基于残缺样本而失真。analyze_data 用于对已聚合的小结果做进阶计算或画图。
"""
